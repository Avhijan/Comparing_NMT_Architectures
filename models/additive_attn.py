...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, query, keys, mask=None):
        ...
        if mask is not None:
            # mask: (batch, src_len, 1), True where PAD -> block attention there
            scores = scores.masked_fill(mask, float('-inf'))
        weights = torch.softmax(scores, dim=1)
        context = torch.bmm(weights.transpose(1, 2), keys)
        return context, weights

class AdditiveAttentionSeq2Seq(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        # (batch, src_len, 1) True at PAD positions -> block attention there
